...
def merge_with_n2pairs(n2pair_df, merged_df):
    ...
    print(f"After merging annotation with n2pair dataframe:\n")
    sys.stdout.flush()  
    print(final_df.head())
...
def main(args):
    ...
    final_output_file = join(annotation_dir, pop, f"final-{pop}-rv.mergedannotation.plusN2pair.thresbased.tsv")
    ...
    output_file = join(annotation_dir, pop, f"final-{pop}-rv.mergedannotation.thresbased.tsv")
    ...
    final_output_file = join(annotation_dir, pop, f"final-{pop}-rv.mergedannotation.plusN2pair.thresbased.stdscaled.tsv")
    final_sorted_df.to_csv(final_output_file, sep='\t', index=False)
    print(f"\n Merged Annotation file with N2 pairs saved to: {final_output_file}")
    sys.stdout.flush()  
    ...
    final_output_filt_file = join(annotation_dir, pop, f"final-{pop}-rv.mergedannotation.plusN2pair.thresbased.stdscaled.annotfiltered.tsv")
