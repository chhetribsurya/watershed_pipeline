...
find_common_samples() {
    ...
    # Check if the covariate file is gzipped or not
    if [[ "$covariate_file" == *.gz ]]; then
        #echo "Covariate file is gzipped. Using zcat to read the file."
        local covariate_samples=$(zcat "$covariate_file" | head -n 1 | tr '\t' '\n' | tail -n +2)
    else
        #echo "Covariate file is plain text. Using cat to read the file."
