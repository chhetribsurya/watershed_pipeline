...
include { CALL_ENRICH_PLOTS           } from './subworkflows/generate_plots' params(params)
...
include { CALL_AUPR_PLOTS             } from './subworkflows/generate_plots' params(params)
...
workflow {
    main:
        ...
        //exp_outlier_file_ch = CALL_OUTLIERS.out[0]
        pval_exp_outlier_file_ch = CALL_OUTLIERS.out[5]
        zscore_exp_outlier_file_ch = CALL_OUTLIERS.out[6]
        gene_rv_file_ch = CALL_RAREVARIANTS.out[9]

        CALL_ENRICH_PLOTS(pops_ch, gene_rv_file_ch, pval_exp_outlier_file_ch, zscore_exp_outlier_file_ch)
        ...
        pval_thresh_channel = CALL_OUTLIERS.out[4]
        WATERSHED_MODELRUN(pops_ch, processed_merged_annotfile_ch, pval_thresh_channel)

        // DEFINE AUPR PLOT PARAMETERS
        processed_merged_annotfile_ch = PROCESS_MERGED_ANNOTATION.out[0]
        
        eval_rds_ch = WATERSHED_MODELRUN.out[0]
        pred_rds_ch = WATERSHED_MODELRUN.out[1]
        CALL_AUPR_PLOTS(pops_ch, eval_rds_ch, pred_rds_ch, processed_merged_annotfile_ch)
