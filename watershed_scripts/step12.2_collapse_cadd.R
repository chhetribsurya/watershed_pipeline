...
# COLLAPSE CADD FILE -- FULL Chromosome
...
# gene level annotation outfile
out.file ='/scratch16/abattle4/surya/datasets/WatershedAFR/data/annotation/cadd/gene-GLOBAL-rv.CADD.collapse.tsv'

# variant level annotation outfile
cadd.outfile = "/scratch16/abattle4/surya/datasets/WatershedAFR/data/annotation/cadd/gene-GLOBAL-rv.CADD.uncollapsed.rvpair.tsv"
...
# READ CADD file and concatenate to a single whole chromosome cadd file
# Initialize an empty list to store individual data tables
all_data <- list()

# Loop through chromosomes 1 to 22
for(chrom in 1:22) {
  print(paste0("Processing chrom",chrom))
  # Create the full file path based on the chromosome number
  cadd.file <- paste0('/scratch16/abattle4/surya/datasets/WatershedAFR/data/annotation/cadd/gene-GLOBAL-rv.CADD.chr', chrom, '.tsv.gz')
  
  # Read the file into a data table and append to the list
  all_data[[chrom]] <- fread(cadd.file)
}

# Concatenate all data tables in the list
cadd_data <- rbindlist(all_data)

# Processing full concatenated cadd data with whole chrom file
cat("\nProcessing cadd annot for full chrom file ...\n")
cadd = cadd_data %>%
  rename("Chrom" = "#Chrom") %>%
...
cadd.collapse_init <- left_join(rv, cadd, by = c("Chrom" = "Chrom", "Start" = "Pos")) %>%
...
print("Gene Level Imputation Done!")

# Write gene-level output
fwrite(cadd.collapse_grouped, out.file, quote = F, sep = '\t', row.names = F, col.names = TRUE)

print("Writing of Gene level annotation done!")

# copy gene-level annotation file to target annotation dir:
file.copy(out.file, genelevel_dir)
...
cadd.collapse$variantID <- paste(cadd.collapse$GeneName, paste(cadd.collapse$Chrom, cadd.collapse$End, cadd.collapse$Ref.x, cadd.collapse$Alt.x, sep = "_"), sep=":")
...
print("Variant Level Concatenation Done!")

# 4. Write selected Gene-Individual-Variant level data
fwrite(cadd.collapse_select, cadd.outfile, quote = F, sep = '\t', row.names = F, col.names = TRUE)

print("Writing of Variant level annotation done!")
cat("\n")

# copy variant-level annotation file to target annotation dir:
file.copy(cadd.outfile, variantlevel_dir)
