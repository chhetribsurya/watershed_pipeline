...
def concatenate_files(input_file):
    """
    Concatenate chromosome-specific files into a single dataframe, ensuring the header is only included once.
    
    Parameters:
    - input_file (str): An example input file from which to infer the file pattern.
    
    Returns:
    - concatenated_df (DataFrame): Concatenated dataframe of all chromosome files.
    """
    
    # Derive the base directory and filename pattern from the input_file
    base_directory = os.path.dirname(input_file)
    base_filename = os.path.basename(input_file).replace('.vcf.gz', '.chr{}.info.tsv')
    filename_pattern = os.path.join(base_directory, base_filename)
    ...
    for i in range(1, 23):
        file_path = filename_pattern.format(i)
...
def main():
    ...
    parser.add_argument(
        "--outfile", required=True, help="Provide output file name with full path"
    )
    ...
    # Generate chr.info.tsv strings to check if *chr*.info.tsv file 
    # already exists, else, info field from VCFs parsed to dataframe
    #df_path_list = [
    ...
    # Concatenate all chromosome file into single to increase power of annoation presence
    info_df = concatenate_files(filename)
    ...
    anno_snv_df.to_csv(outputfile, sep="\t", index=False)
