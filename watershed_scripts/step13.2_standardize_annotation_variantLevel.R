# standardize all of the columns except the ones called GeneName, SubjectID and variantID
...
input2 <- input %>%
  mutate_at(vars(-SubjectID, -GeneName, -variantID), ~(scale(., center=TRUE, ) %>% as.vector))

fwrite(input2,file=output.file, sep="\t", col.names = TRUE)
