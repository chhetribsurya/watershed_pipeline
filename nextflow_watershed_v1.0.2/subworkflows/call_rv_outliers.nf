...
workflow CALL_OUTLIERS {
    ...
    if (params.analysis == "GLOBAL") {
        ...
        combined_ch.view({ pops_norm, lognorm_expr, peer_factors, covariates -> 
            "Final combined output: ($pops_norm, $lognorm_expr, $peer_factors, $covariates)"
        })
        ...
        if (params.mode == 'entry') {
            ...
            call_expr_outliers_only.out.outliers.view({"Here's the outlier file list for solo mode: $it"})
            call_expr_outliers_only.out.pops_outliers.view({"Here's the outlier pop for solo mode: $it"})
            ...
            //Channel.fromPath("$params.rv_file").set { gene_rarevar_ch }
            ...
            combined_ch.view { pops_peer, residual_expr, gene_rarevar ->
                "Final Global outlier call step combined output: ($pops_peer, $residual_expr, $gene_rarevar)"
            }
