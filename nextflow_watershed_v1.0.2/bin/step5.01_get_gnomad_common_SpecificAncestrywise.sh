...
for i in {1..22};
do
        echo bcftools view -r chr$i --output-file ${gnomad_dir}/gnomad.padded10kb_PCandlinc_only_chr${i}.vcf.gz --output-type z $gnomad
        echo bcftools index --tbi ${gnomad_dir}/gnomad.padded10kb_PCandlinc_only_chr${i}.vcf.gz
        echo $i
done
...
get_common() {
        ...
        if [ $pop == "AFR" ]; then
                bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_afr >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad
                
        elif [ $pop == "EUR" ]; then
               bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_nfe >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad

        elif [ $pop == "AMR" ]; then
               bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_amr >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad

        elif [ $pop == "EAS" ]; then
               bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_eas >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad

        elif [ $pop == "SAS" ]; then
               bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_oth >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad

        elif [ $pop == "GLOBAL" ]; then
                bcftools view --regions-file $kgpex_rare_bed --include 'INFO/AF_afr >= 0.01 || INFO/AF_amr >= 0.01 || INFO/AF_eas >= 0.01 || INFO/AF_nfe >= 0.01 || INFO/AF_oth >= 0.01' \
                        -Oz -o ${gnomad_dir}/gnomad.${pop}_common_$chr.vcf.gz $gnomad
        else
                echo "Population is neither AFR,EUR,AMR,EAS,SAS,GLOBAL. Exiting"
                exit 1
        fi 
