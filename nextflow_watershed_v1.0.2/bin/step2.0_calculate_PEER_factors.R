...
## Actual running of PEER
## Takes multiple arguments from the command line
...
GetPeerModel <- function(m1, MaxFactorsN, MaxIterations, a_pa, a_pb, e_pa, e_pb){
	...
	if (!is.null(res)) RaiseError("not correct")
...
FactorsNComparison <- function(m1, MaxIterations, a_pa, a_pb, e_pa, e_pb, Factors){
	...
	plot(1.0 / PEER_getAlpha(model), xlab = "Factors", ylab = "Factor relevance",
             type = PlotType, xlim = c(0, length(PEER_getAlpha(model))), ylim = c(0,1000),
             main = sprintf("Factors: %s\n", k))
    }
    dev.off()
}
...
WritePeerData <- function(m1, model, MaxIterations, a_pa, a_pb, e_pa, e_pb, MaxFactorsN){
	...
	WritePeerDataCore(W, TranscriptIds, FactorsTags, "weights")
	...
	cat(sprintf("%s\t%s\n", "ID", "data"), file = "PeerInfo.tsv")
	cat(sprintf("%s\t%s\n", "MaxIterations", MaxIterations), file = "PeerInfo.tsv", append = T)
	cat(sprintf("%s\t%s\n", "MaxFactorsN", MaxFactorsN), file = "PeerInfo.tsv", append = T)
	cat(sprintf("%s\t%s\n", "e_pa", e_pa), file = "PeerInfo.tsv", append = T)
	cat(sprintf("%s\t%s\n", "e_pb", e_pb), file = "PeerInfo.tsv", append = T)
	cat(sprintf("%s\t%s\n", "a_pa", a_pa), file = "PeerInfo.tsv", append = T)
	cat(sprintf("%s\t%s\n", "a_pb", a_pb), file = "PeerInfo.tsv", append = T)
	cat(sprintf("ResidualsSquareMean\t%s\n", mean((Yc)**2)), file = "PeerInfo.tsv", append = T)
}

PlotFactorsVariance <- function(alpha, prefix){
	# plot variance of factors - in this case, we expect
	# a natural elbow where there are 5 active factors, as 5 were simulated
	pdf(paste(prefix, "_FactorsVariance.pdf", sep = ''), width = 8, height = 8)
	plot(1.0 / alpha, xlab = "Factors", ylab = "Factor relevance", main = "")
	lines(1.0 / alpha, xlab = "Factors", ylab = "Factor relevance", main = "")
	dev.off()
}

PlotFactorsWeights <- function(model, prefix){
	pdf(paste(prefix, '_FactorsWeights.Scatterplot.pdf', sep = ''))
...
if (length(args) < 4) {
    cat("Usage: Rscript calculate_PEER_factors.R TraitsFileName MaxFactorsN MaxIterations BoundTol VarTol e_pa e_pb a_pa a_pb\n", file = stderr())
    quit(status = 2)
}
