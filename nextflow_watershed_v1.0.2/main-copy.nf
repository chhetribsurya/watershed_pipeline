...
workflow {
    main:
        ...
        CALL_OUTLIERS(pops_ch, subjids_channel)

        //DEFINE ANNOTATION CHANNELS
        Channel.value("$params.cohort_name").set { pops_ch }
        Channel.fromPath("$params.rv_file_bed").set { gene_rarevar_ch } //annotation ".rv.bed" file
        Channel.fromPath("$params.rv_file").set { gene_rarevar_ch2 } //annotation ".rv.txt" file
        ...
        COLLAPSE_ANNOTATION(pops_ch, rv_file_ch, vep_loftee_file_ch, unique_cadd_files_dir_ch, ucsc_phylop_file_channel, gencode_file_channel)

        CALL_N2_PAIRS(pops_ch, rv_file_ch, exp_outlier_file_channel)
